...
class OneTimeAlarmActivity : AppCompatActivity(), View.OnClickListener, DatePickerFragment.DialogDateListener, TimePickerFragment.DialogTimeListener {
    ...
    override fun onClick(v: View) {
        when (v.id) {
            ...
            R.id.btn_set_time_one_time -> {
                ...
                timePickerFragmentOneTime.show(supportFragmentManager, TIME_PICKER_ONE_TAG)
            }
    ...
    override fun onDialogTimeSet(tag: String?, hourOfDay: Int, minute: Int) {
        ...
        when (tag) {
            TIME_PICKER_ONE_TAG -> tv_once_time.text = dateFormatOneTime.format(calendar.time)
            else -> {
